...
export class ProductsComponent implements OnInit {
  ...
  get_all_posts(page?: any, limitChanged?: any, LoadMore?: any) {
    ...
    if(this.sendCategories){
      this.cond.sendCategories = this.sendCategories;
    }
    if(this.sendTags){
      this.cond.sendTags = this.sendTags;
    }
    ...
    this.ResourcesService.getProductPosts(this.cond).subscribe(
      (res) => {
        if (res && res.data) {
  ...
  get_traiing_products(page?:any, limitChanged?:any,LoadMore?:any) {
    ...
    this.ResourcesService.getTrainingProducts(this.cond).subscribe(res => {
      if (res && res.data) {
        // console.log(res.data);
        const AllTrainings = (res.data.length) ? res.data : [];
        const trainingDetails = AllTrainings.find(obj => {
          return obj.slug == this.ProductSlug;
        });
        if(trainingDetails){
          this.getResourcesCategories(trainingDetails.ID,trainingDetails.product_type_id);
        }
      }else{
  ...
  getResourcesCategories(productId:any, productTypeId:any) {
    ...
    this.ResourcesService.getResourcesCategories(this.cond).subscribe(
      (res) => {
  ...
  getResourcesTags() {
    ...
    this.ResourcesService.getResourcesTagsMember(cond)
      .subscribe(res => {
        if (res && res.data && res.data.length) {
            this.resourcestagsList = res.data;
            console.log("this.resourcestagsList",this.resourcestagsList);
        }
      }, (err) => {});
  }
  ...
  getRecentVideos() {
    ...
    this.RecentVideos = [];
    this.VideoService.getRecentVideos(this.cond).subscribe(
      (res) => {
        if (res && res.data && res.data.length) {
          this.RecentVideos = res.data && res.data.length ? res.data : [];
          this.user_likes =
            res.user_likes && res.user_likes.length ? res.user_likes : [];
  ...
  LikeVideo(ID: Number, Liked: any) {
    ...
    this.VideoService.LikeVideo(this.cond).subscribe(
      (res) => {
        // Check data
        if (res && res.data) {
          _.map(this.RecentVideos, function (v) {
  ...
  AddtoLibrary(ID: any) {
    ...
    this.VideoService.AddtoLibrary(cond).subscribe(
      (res) => {
        // Check data
        if (res && res.data) {
  ...
  Watchlater(ID: any) {
    ...
    this.VideoService.Watchlater(cond).subscribe(
      (res) => {
        // Check data
        if (res && res.data) {
          this.toastr.success("Added Successfully.");
  ...
  // Select tag on click
  onClickTagSelected(tagId:number) {
