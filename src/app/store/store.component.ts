...
export class StoreComponent implements OnInit {
  ...
  checkKeyParam() {
    ...
    if(this.route.snapshot.params.channel){
  ...
  SearchByKeyword(): void {
    if (this.channelSlug) {
      if (this.Keyword) {
        this.changeRouter("shop/"+this.channelSlug+"?q=" + this.Keyword);
      } else {
        this.changeRouter("shop/"+this.channelSlug);
      }
    }else{
      if (this.Keyword) {
  ...
  get_products_list(page?: any, limitChanged?: any, LoadMore?: any) {
    ...
    this.cond.marketplace = false;

    this.cond.channelSlug = this.channelSlug;
