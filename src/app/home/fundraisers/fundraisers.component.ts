...
import { FundraisersService } from "../../services/fundraisers/fundraisers.service";
...
@Component({
  ...
  styleUrls: ["./fundraisers.component.sass"],
})
export class FundraisersComponent implements OnInit {
  ...
  FUNDRAISING_HOST: string = environment.config.FUNDRAISING_HOST;
  AllFundraiers: Array<any>;
  newUUid: string;
  UserToken: string;
  userData: any;
  ...
  getFundraisers() {
    ...
    this.FundraisersService.getFundraisers(this.cond).subscribe(
      (res) => {
        if (res && res.data) {
          this.AllFundraiers = res.data.length ? res.data : [];
        } else {
          this.AllFundraiers = [];
        }
      },
