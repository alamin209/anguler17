...
export class ResourcesComponent implements OnInit {
  ...
  checkKeyParam(): void {
    ...
    if (this.getParams && this.getParams.category) {
      // set price params
      this.CategorySlug = this.getParams.category;
    }
  ...
  onLoadMore() {
    ...
    if (!this.AllResourcesLoaded) {
      ...
      this.get_all_resources(this.page, this.limit, true);
    }
  }
      ...
      (res) => {
        if (res && res.data && res.data.length) {
          ...
          if (this.CategorySlug) {
            var lastSlash = this.CategorySlug.lastIndexOf("-");
            let cID = this.CategorySlug.substring(lastSlash + 1);

            setTimeout(() => {
              //jQuery('#'+cID).click();
            }, 2000);
            this.applyFilters(true, cID);
          }
        } else {
    ...
    this.VideoService.getRecentVideos(this.cond).subscribe(
      (res) => {
        if (res && res.data && res.data.length) {
          ...
          console.log(this.globals.videoASOptions);
          // Copy objects
